...
namespace WFAYilanOyunu
{
    public partial class Form1 : Form
    {
        ...
        private bool YilaninKuyruguUzerindeMi(Point hedef)
        {
        ...
        {
            //e.Graphics.FillRectangle(Brushes.Green, rnd.Next(300), rnd.Next(300), 40, 40);
            //Point birNokta = new Point(3,4);
            //BogumCiz(e.Graphics, birNokta, Color.Orange);

            //Point baskaBirNokta = new Point(6, 7);
            //BogumCiz(e.Graphics, baskaBirNokta, Color.Orange);
        ...
        {
            BogumCiz(graphics, yem, Color.DarkRed);
        }
        ...
        private void HareketEt()
        {
            ...
            // OYUN BÝTTÝ MÝ?
            if (YilaninKuyruguUzerindeMi(yeniBas) || SahaDisindaMi(yeniBas))
            {
                timer1.Stop();
            ...
            yilan.Insert(0, yeniBas); //Listenin sýfýrýncý elmanýna ekle

            if (yemiYuttuMu)
            {
                yemeAdet++;
                lblPuan.Text = $"Puan: {yemeAdet * 100:00000}";
                timer1.Interval = (int)(timer1.Interval * .90);
                YemOlustur();
            }
            else
            {
